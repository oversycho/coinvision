                ...
                children: [
                  Padding(
                    ...
                    child: Row(
                      children: [
                        ...
                        Text(
                          isRtl ? 'تاریخچه سود تحقق‌یافته' : 'Realized P&L History',
                          style: AppFonts.display(color: colors.fg, size: 18, weight: FontWeight.w800),
                        ),
                      ],
                    ),
                  ),
                  Padding(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(color: colors.muted, borderRadius: BorderRadius.circular(18), border: Border.all(color: colors.border)),
                      child: Column(
                        children: [
                          Text((isRtl ? 'مجموع' : 'Total').toUpperCase(), style: AppFonts.display(color: colors.mutedFg, size: 10, letterSpacing: 0.2)),
                          const SizedBox(height: 4),
                          Text(
                            '${total >= 0 ? '+' : ''}${Tr.formatPrice(total, lang)} ${Tr.t('toman', lang)}',
                            style: AppFonts.display(color: total >= 0 ? colors.gain : colors.loss, size: 22, weight: FontWeight.w900),
                          ),
                        ],
                      ),
                    ),
                        ...
                        ? Center(
                            child: Column(
                              ...
                              children: [
                                ...
                                Text(isRtl ? 'هنوز فروشی ثبت نشده' : 'No sells recorded yet', style: TextStyle(color: colors.mutedFg, fontSize: 13)),
                              ],
                            ),
                          )
                        : ListView.builder(
                                    ...
                                    children: [
                                      ...
                                      Expanded(
                                        child: Column(
                                          ...
                                          children: [
                                            Text('${e.coinSymbol} — ${e.amount.toStringAsFixed(6)}',
                                                style: AppFonts.display(color: colors.fg, size: 13, weight: FontWeight.w700)),
                                            const SizedBox(height: 2),
                                            Text(
                                              e.avgCost != null
                                                  ? '${isRtl ? 'خرید' : 'cost'}: ${Tr.formatPrice(e.avgCost!, lang)} → ${isRtl ? 'فروش' : 'sell'}: ${Tr.formatPrice(e.sellPrice, lang)}'
                                                  : (isRtl ? 'بدون قیمت خرید مرجع' : 'no cost basis on record'),
                                              style: TextStyle(color: colors.mutedFg, fontSize: 11),
                                            ),
                                            const SizedBox(height: 2),
                                            Text(Tr.formatTime(e.recordedAt, lang), style: AppFonts.mono(color: colors.mutedFg, size: 10)),
                                          ],
                                        ),
                                      ),
                                      Text(
                                        '${isProfit ? '+' : ''}${Tr.formatPrice(e.pnl, lang)}',
                                        style: AppFonts.mono(color: isProfit ? colors.gain : colors.loss, size: 13, weight: FontWeight.w700),
                                      ),
                                    ],
                                  ),
                                ),
